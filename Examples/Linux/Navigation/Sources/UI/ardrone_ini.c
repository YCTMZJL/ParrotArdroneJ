...
static int autoflyflag =  0; //zidong=1 shoudong  =0 ###############################
...
else
{
    ...
    stick2UD = center_y2;
      ...
      int Stage1time = STARTPOINT + 300 ;
      int Stage2time = Stage1time + 120 ;
      int Stage3time = Stage2time +60 ;//weiding
      int Stage4time = Stage3time +40 ;//xiajiang?
 ...
 /*	int i;
	if(startcount == 1000)
	{
		for(i=0; i<5; i++)
		{
		  ardrone_at_set_progressive_cmd_with_magneto(1, 0.0, 0.6, 0.0, 0.0, 0.0, 0.0);//?had some question
		}
	}
	//zhi jie chuan zhi ling ; 80 chabuduo shi liangmiao
//*/
      ...
      else if(startcount > Stage3time  && startcount <= Stage4time ) { //wending
      	stick2UD = center_y2 + 32767*0.6 ;//hou mian you fu hao
      }
     else if(startcount >  Stage4time +30)
       {   
       	//ardrone_academy_navdata_emergency();
       	 autoflyflag  =  0;
       }
